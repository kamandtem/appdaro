...
import { Check, Clock, AlertTriangle, Pill, Droplet, Syringe, Pipette, Bandage, CheckCircle2, ClipboardList, X, Hourglass, ShieldAlert, Ban, Timer } from 'lucide-react';
...
export const StackedCards: React.FC<StackedCardsProps> = ({
  ...
  return (
    <>
      {showTutorial && (
        <div className="fixed inset-0 z-[90] bg-slate-900/70 backdrop-blur-sm flex flex-col items-center justify-center gap-4 px-6 text-center animate-in fade-in duration-300 overflow-y-auto py-8">
          <div className="text-white max-w-xs mx-auto">
            <p className="text-lg sm:text-xl font-black mb-3">راهنمای دکمه‌های کارت</p>

            <div className="text-right space-y-3 text-sm text-white/90">
              <div className="flex items-start gap-2">
                <Check className="w-4 h-4 text-emerald-400 mt-0.5 shrink-0" strokeWidth={3} />
                <span><b>مصرف کردم:</b> نوبت ثبت می‌شه و می‌ره توی سوابق مصرف.</span>
              </div>
              <div className="flex items-start gap-2">
                <Ban className="w-4 h-4 text-rose-400 mt-0.5 shrink-0" strokeWidth={2.5} />
                <span><b>رد کردم:</b> نوبت میره توی بخش مصرف‌نشده‌ها.</span>
              </div>
              <div className="flex items-start gap-2">
                <Timer className="w-4 h-4 text-amber-400 mt-0.5 shrink-0" strokeWidth={2.5} />
                <span><b>بعداً:</b> نوبت به تعویق می‌افته و طبق یه زمان‌بندی یادت می‌ندازیم.</span>
              </div>
            </div>

            <div className="text-right bg-white/10 rounded-2xl p-3 mt-3 text-xs text-white/80 leading-relaxed">
              مثال برای دارویی با فاصله‌ی ۸ ساعته: یه یادآور ربع‌ساعت بعد از نوبت، یکی ۲ ساعت بعد، و آخری ۴ ساعت بعد (که آخرین فرصته). بعد از اون دیگه اون نوبت رو نزن، منتظر نوبت بعدی بمون.
            </div>
            <p className="text-[11px] text-white/60 mt-2">
              این یه قاعده‌ی کلیه؛ برای داروهای خاص خودتون حتماً از پزشک بپرسید اگه نوبتش به تعویق افتاد چیکار کنید.
            </p>
          </div>
          <button
            type="button"
              ...
              <div>
                <div className="flex items-center justify-between gap-2 mb-4">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {activeIsSnoozed && (
                      <div
                        title="این نوبت را «بعداً» گذاشته‌اید"
                        className="w-9 h-9 flex items-center justify-center rounded-full bg-amber-50/90 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800/80 text-amber-600 dark:text-amber-400 shadow-sm"
                      >
                        <Timer className="w-4 h-4" strokeWidth={2.25} />
                      </div>
                    )}
                    <button
                      type="button"
