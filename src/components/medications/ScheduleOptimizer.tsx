...
// کلید localStorage برای اینکه توضیح این قابلیت فقط بار اول به کاربر نمایش داده شود.
// عمداً مستقل و مجزا از storageService/AppState نگه داشته شده تا این ویژگی کاملاً
// self-contained بماند و هیچ تداخلی با ذخیره‌سازی اصلی برنامه نداشته باشد.
const INTRO_SEEN_KEY = 'daroto_schedule_optimizer_intro_seen';
...
function markIntroSeen(): void {
  try {
    ...
    // اگر localStorage در دسترس نبود (مثلاً حالت خصوصی مرورگر)، صرفاً یعنی توضیح
    // ممکن است دوباره نمایش داده شود؛ چیزی نباید بشکند یا خطا بدهد.
  }
}
...
export const ScheduleOptimizer: React.FC<ScheduleOptimizerProps> = ({ times, intervalHours, scheduleStartAt, onApply, onRestore, hasBackup }) => {
  ...
  // فقط بار اولی که کاربر روی این قابلیت می‌زند «توضیح این بخش چیست» نمایش داده می‌شود؛
  // دفعات بعد مستقیماً پیشنهاد نمایش داده می‌شود.
  ...
  return <>
    <button type="button" onClick={launch} className="w-full flex items-center justify-center gap-2 rounded-xl border border-violet-200 bg-violet-50/70 px-3 py-2 text-xs font-bold text-violet-700">
    ...
    {open && <div className="fixed inset-0 z-[70] flex items-end justify-center bg-slate-900/50 p-3 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[28px] bg-white p-5 shadow-2xl dark:bg-slate-900">
        ...
        {showIntro ? <><p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">سعی می‌کند نوبت‌ها را تا جای ممکن از بازه معمول خواب، ۱۲ شب تا ۵ صبح، دور کند؛ تعداد نوبت‌ها و فاصله‌هایشان تغییر نمی‌کند.</p><button type="button" onClick={dismissIntro} className="mt-5 w-full rounded-xl bg-violet-600 py-3 font-black text-white">متوجه شدم</button></> : <>
